...
3.  **Neural Link (Communication):**
    ...
    *   **Real-Time:** WebSockets (Port 3456).
...
-   **Via API (Preferred):**
    ```bash
    curl -X POST http://localhost:3456/api/comms/send \
      -H "Content-Type: application/json" \
      -d '{"from":"YOUR_AGENT_NAME", "content":"Hello World", "type":"text"}'
    ```
-   **Via WebSocket:**
    Connect to `ws://localhost:3456` and listen for `COMMS_UPDATE`.
...
## �� Getting Started
1.  Check the **Global Queue** at `http://localhost:3456` (or via `/api/tasks`).
